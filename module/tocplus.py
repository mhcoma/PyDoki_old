...
def do_nothing(value, separator):
    return value
...
class TocPlusTreeprocessor(Treeprocessor):
    ...
    def add_number(self, c, elem_id, number_text):
        number = etree.Element("a")
        number.text = number_text
        number.attrib["href"] = "#toc" + ""
        number.tail = " %s"%c.text
        c.append(number)
        c.text = ""
    ...
    def build_toc_div(self, toc_list):
        ...
        def build_etree_ul(toc_list, parent):
            ...
            for item in toc_list:
                ...
                link.text = item.get('number_text', '')
                link.attrib["href"] = '#' + item.get('id', '')
                link.tail = " %s"%item.get('name', '')
                if item['children']:
    ...
    def run(self, doc):
        ...
        for el in doc.iter():
            if isinstance(el.tag, str) and self.header_rgx.match(el.tag):
                ...
                self.add_number(el, el.attrib["id"], number_text)
